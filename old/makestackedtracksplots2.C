...
void makestackedtracksplots2(  ) {
  ...
  TString outputlocation = "output/tracksplots/";//where the output will be saved
  TString plotfilename = "tracksplots.pdf";//name of file that will hold drawn plots
  ...
  for(int ci =0; ci<ncanvases; ci++){ //loop over canvases
    ...
    TString Bdecay;
    TString branch;
    if((int)floor(ci/2)%2 ==0){//every 3rd canvas
      B0=0;
