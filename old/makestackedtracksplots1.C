...
void makestackedtracksplots1(  ) {
  ...
  TString outputlocation = "output/stackedtracksplots/";//where the output will be saved
  TString plotfilename = "stackedtracksplots.pdf";//name of file that will hold drawn plots
  ...
  for(int ci =0; ci<ncanvases; ci++){ //loop over canvases
    ...
    TString filetype;
    if((int)floor(ci/2)%2==0){//true every 3rd canvas
      filetype = "data";
    }else filetype = "MC";
