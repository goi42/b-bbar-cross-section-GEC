...
void plotbranches(TString runmode = "d", TString drawopt = "norm"){
  ...
  cout<<" or 'f' to select the files and trees to include."<<endl;
  ...
  vector< vector<TString> > branch;
  branch.resize(nFiles, vector<TString>(nBranches));
  for(int i=0;i<nFiles;i++) branch[i][0]="nLongTracks";
  ...
  hiBin[0]=262;
  vector< vector<TString> > cut;
  cut.resize(nFiles, vector<TString>(nBranches));
  for(int bi=0;bi<nBranches;bi++) for(int fi=0;fi<nFiles;fi++) cut[fi][bi]="";
  ...
  if(runmode.Contains("f")){
    ...
    cout<<"how many file types? ";
    nFileTypes=0;
    while(!(nFileTypes==1||nFileTypes==2)){
      cout<<"1 or 2?"<<endl;
      cin>>nFileTypes;
    }
    ...
    for(int yi=0; yi<nYears; yi++){
      for(int fti=0; fti<nFileTypes; fti++){
        for(int di=0; di<nDecays; di++){
          ...
          if(yi==0&&fti==0&&di==0) cout<<"Specify the folder and tree for your decay, e.g., 'Tuple_b2D0MuX/DecayTree':"<<endl;
          cout<<"File "<<fi<<" has tree ";
          cin>>tree[fi];
          fi++;
        }
      }
  ...
  if(runmode.Contains("b")){ //ask for branches
    ...
    for(int bi=0; bi<nBranches; bi++){
      for(int yi=0; yi<nYears; yi++){
        for(int fti=0; fti<nFileTypes; fti++){
          for(int di=0; di<nDecays; di++){
            int fi=yi*nDecays*nFileTypes+fti*nDecays+di;//this only works because of the order files are asked for
            if(response=="y") cout<<"for file "<<fi<<", corresponding to "<<year[yi]<<", "<<filetype[fti]<<", "<<decay[di]<<", ";
  ...
  if(runmode.Contains("c")){ //ask for cuts
    TString fbd="";
    cout<<"do the cuts vary by file, branch, decay, some/all, or none? (f,b,d,f/b/d,n)"<<endl;
    cin>>fbd;
    while(!(fbd.Contains("f")||fbd.Contains("b")||fbd.Contains("d")||fbd.Contains("n")) \
          ||((fbd.Contains("f")||fbd.Contains("b")||fbd.Contains("d"))&&fbd.Contains("n"))\
          ||(fbd.Contains("f")&&fbd.Contains("b"))){
      cout<<"f,b,d,f/b/d,n?";
      if(fbd.Contains("f")&&fbd.Contains("b")) cout<<" If it varies by file, it varies by branch. Do not include both f&b:"<<endl;
      cout<<endl;
      cin>>fbd;
    }
    cout<<"what cuts would you like to make? Include them in one line, e.g., '(x>1)&&(x<3)'"<<endl;
    for(int bi=0; bi<nBranches; bi++){
      for(int yi=0; yi<nYears; yi++){
        for(int fti=0; fti<nFileTypes; fti++){
          for(int di=0; di<nDecays; di++){
            int fi=yi*nDecays*nFileTypes+fti*nDecays+di;//this only works because of the order files are asked for
            if(fbd.Contains("f")) cout<<"for file "<<fi<<", corresponding to "\
                                      <<year[yi]<<", "<<filetype[fti]<<", "<<decay[di]<<": ";
            if((fbd=="d"&&fti==0&&yi==0&&bi==0)||(fbd.Contains("d")&&fti==0&&yi==0)) cout<<decay[di]<<": ";
            if(fbd.Contains("b")) cout<<"for branch "<<branch[fi][bi]<<": ";
            if(!(fbd=="n")) cout<<endl;
            if((fbd=="n"&&fi==0&&bi==0)||(fbd=="f"&&bi==0)||(fbd=="b"&&fi==0)||(fbd=="d"&&fti==0&&yi==0&&bi==0)\
               ||(fbd.Contains("b")&&(fbd.Contains("f")||(fbd.Contains("d")&&fti==0&&yi==0)))){
              cin>>cut[fi][bi];
            }
            if(fbd=="n") cut[fi][bi]=cut[0][0];
            if(fbd=="f") cut[fi][bi]=cut[fi][0];
            if(fbd=="b") cut[fi][bi]=cut[0][bi];
            if(fbd.Contains("d")) {
              int fis=di;//=0*nDecays*nFileTypes+0*nDecays+di
              cut[fi][bi]=cut[fis][bi];
            }
          }
        }
      }
  ...
  for(int i=0;i<4;i++){
    ...
    if(Lresponse[i]=="branch") L[i]=branch[0];//all the branch[i] should be the same length
  ...
  for(int ci=0; ci<nCanvases; ci++){
    ...
    for(int hi=0; hi<nhpc; hi++){
      int file_num=yi*nDecays*nFileTypes+fti*nDecays+di;//this only works because of the order files are asked for
      
      //create convenient strings
      TString histring = Form("%d",hi);
      TString hname = "h"+cistring+histring;
      //create histograms
      h[ci][hi] = new TH1F(hname,branch[file_num][bi],nBins[bi],loBin[bi],hiBin[bi]);
      ...
      placeholder = branch[file_num][bi]+">>"+hname;
      MyTree->Draw(placeholder,cut[file_num][bi],drawopt);
      ...
      for(int i=0;i<4;i++){//loop over hierarchy levels
        ...
        if(yes==kTRUE){//if it is, we want it in the legend entry
          ...
          if(Lresponse[i]=="branch") leglabel+=branch[file_num][bi];
        }
        if(hi==0){//since stacktitle reflects non-compared hierarchy levels, it only needs to change per canvas
          if(yes==kFALSE){//if it's not, we want it in the title
            ...
            if(Lresponse[i]=="branch") stacktitle+=branch[file_num][bi];
          }
        }
  ...
  for(int i=0;i<nBranches;i++){
    for(int j=0;j<nFiles;j++){
      if(cut[j][i]!="") cout<<"in file "<<j<<", branch "<<branch[j][i]<<" had cuts: "<<cut[j][i]<<endl;
    }
  }
